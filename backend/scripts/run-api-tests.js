...
async function main() {
    ...
    if (res.status !== 403 && res.status !== 401)
        throw new Error(`Missing auth should fail but got ${res.status}`);
    ...
    if (res.status !== 200)
        throw new Error("Profile setup failed: " + JSON.stringify(res.data));
    ...
    if (res.data.email || res.data.username)
        throw new Error("Email leaked in public profile!");
    ...
    if (res.status !== 400)
        throw new Error("Reserved handle should be rejected");
    ...
    if (res.status !== 409)
        throw new Error("Duplicate handle should return 409");
    // 3. Slug & Tag Tests
    ...
    if (!slug1)
        throw new Error("Slug not generated");
    if (slug1 !== dynamicSlugPrefix)
        throw new Error(`Unexpected slug: ${slug1} (expected ${dynamicSlugPrefix})`);
    // Publish the article
    ...
    if (res.status !== 200)
        throw new Error(`GET slug1 failed with ${res.status}: ${JSON.stringify(res.data)}`);
    if (res.data.tags.length !== 1)
        throw new Error("Tags were not normalized to a single tag");
    console.log(`Published slug 1: ${slug1} with tags: ${res.data.tags.map((t) => t.tag.name).join(",")}`);
    // Duplicate title
    res = await fetchJSON("/api/v1/blog", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": tokenAlice },
    ...
    if (slug1 === slug2)
        throw new Error("Duplicate titles should not have same slug");
    ...
    if (slug3 === `12345`)
        throw new Error("Numeric title should not produce numeric slug");
    ...
    if (res.data.slug !== slug1)
        throw new Error("Slug changed after edit");
    ...
    if (res.status !== 400)
        throw new Error(`Six tags should be rejected with 400, got ${res.status}`);
    ...
    if (res.data.tags.length !== 0)
        throw new Error("Empty array did not clear tags");
    ...
    if (res.status !== 200)
        throw new Error(`GET slug1 failed with ${res.status}: ${JSON.stringify(res.data)}`);
    if (res.data.tags.length !== 1 || res.data.tags[0].tag.name !== "preserved-tag")
        throw new Error("Omitted tags did not preserve tags");
    // 4. Access testing
    console.log("-> Testing Route Access");
    // Access by slug
    res = await fetchJSON(`/api/v1/blog/${slug1}`);
    if (res.status !== 200 || res.data.id !== id1)
        throw new Error("Access by slug failed");
    // Access by ID (old numeric)
    res = await fetchJSON(`/api/v1/blog/${id1}`);
    if (res.status !== 200 || res.data.id !== id1)
        throw new Error("Access by ID failed");
    // Access unknown slug
    res = await fetchJSON(`/api/v1/blog/unknown-slug-xyz`);
    if (res.status !== 404)
        throw new Error("Unknown slug should be 404");
    // Draft by slug -> 404
    res = await fetchJSON("/api/v1/blog", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": tokenAlice },
    ...
    if (dbDraft?.slug) {
        ...
        if (res.status !== 404)
            throw new Error("Draft accessed by slug should be 404");
    }
    // 5. Search & Pagination
    ...
    if (!res.data.articles.some((a) => a.slug === slug2))
        throw new Error(`Search by tag failed: ${JSON.stringify(res.data)} (expected slug2=${slug2})`);
    ...
    if (res.data.articles[0].id === paginatedId)
        throw new Error("Pagination duplicated records");
    // 6. Related articles
    ...
    if (res.status !== 200)
        throw new Error(`Tag update 1 failed: ${JSON.stringify(res.data)}`);
    res = await fetchJSON("/api/v1/blog", {
        method: "PUT",
        headers: { "Content-Type": "application/json", "Authorization": tokenAlice },
    ...
    if (res.status !== 200)
        throw new Error(`Tag update 2 failed: ${JSON.stringify(res.data)}`);
    res = await fetchJSON(`/api/v1/blog/${slug1}/related`);
    if (!res.data.articles.some((a) => a.id === id2))
        throw new Error(`Related articles missing shared tag article: ${JSON.stringify(res.data.articles.map((a) => a.id))}`);
    if (res.data.articles.some((a) => a.id === id1))
        throw new Error("Related articles included self");
    // 7. Sitemap XML
    console.log("-> Testing Sitemap XML");
    res = await fetchJSON("/sitemap.xml");
    if (res.status !== 200 || !res.text.includes(slug1))
        throw new Error("Sitemap missing slug");
    if (dbDraft?.slug && res.text.includes(dbDraft.slug))
        throw new Error("Sitemap includes draft");
